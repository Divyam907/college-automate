...
export default function CollegeLayout({ user, onLogout }) {
  ...
  const drawer = (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#1e293b', color: '#f1f5f9' }}>
      {/* Brand */}
      <Box sx={{ p: 3, textAlign: 'center' }}>
        ...
        <Typography variant="subtitle1" sx={{ fontWeight: 700, letterSpacing: 0.5, color: '#fff' }}>
          College Portal
        </Typography>
            ...
            <ListItem key={item.text} disablePadding sx={{ mb: 0.5 }}>
              ...
              >
                ...
                <ListItemText primary={item.text}
                  sx={{ '& .MuiListItemText-primary': { fontSize: '0.875rem', fontWeight: active ? 600 : 400, color: active ? '#fff' : '#cbd5e1' } }}
                />
              </ListItemButton>
            </ListItem>
          )
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: theme.palette.background.default }}>
      ...
      {isMobile ? (
        <Drawer variant="temporary" open={mobileOpen} onClose={() => setMobileOpen(false)}
          sx={{ '& .MuiDrawer-paper': { width: drawerWidth, boxSizing: 'border-box', border: 'none', borderRadius: 0, bgcolor: '#1e293b' } }}>
          {drawer}
        </Drawer>
      ) : (
        <Drawer variant="permanent"
          sx={{ width: drawerWidth, flexShrink: 0,
                '& .MuiDrawer-paper': { width: drawerWidth, boxSizing: 'border-box', border: 'none', borderRadius: 0, bgcolor: '#1e293b', overflowX: 'hidden' } }}>
          {drawer}
        </Drawer>
      )}
        ...
        <AppBar position="sticky" elevation={0} sx={{ bgcolor: theme.palette.background.paper, borderBottom: `1px solid ${theme.palette.divider}` }}>
          <Toolbar>
            ...
            <IconButton onClick={toggleTheme} sx={{ mr: 1 }}>
              {mode === 'dark' ? <LightMode sx={{ color: '#fbbf24' }} /> : <DarkMode sx={{ color: 'text.secondary' }} />}
            </IconButton>
            <Chip
