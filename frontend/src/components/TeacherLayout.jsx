...
export default function TeacherLayout({ user, onLogout }) {
  ...
  const drawer = (
    <Box sx={{
      ...
      bgcolor: '#1e293b',
      color: '#f1f5f9',   /* ← CSS inheritance: all child text becomes white */
      overflow: 'hidden',
    }}>
      {/* Brand */}
          ...
          return (
            <ListItem key={item.text} disablePadding sx={{ mb: 0.5 }}>
              ...
              >
                ...
                <ListItemText
                  primary={item.text}
                  sx={{ '& .MuiListItemText-primary': { color: active ? '#fff' : '#cbd5e1', fontSize: '0.9rem', fontWeight: active ? 700 : 500 } }}
                />
              </ListItemButton>
            </ListItem>
          )
      ...
      <Box sx={{ p: 2, borderTop: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        <ListItemButton onClick={onLogout} sx={{ borderRadius: 1.5, py: 0.8 }}>
          <ListItemIcon sx={{ color: '#f87171', minWidth: 32 }}><Logout fontSize="small" /></ListItemIcon>
          <ListItemText
            primary="Logout"
            sx={{ '& .MuiListItemText-primary': { color: '#f87171', fontSize: '0.85rem' } }}
          />
        </ListItemButton>
      </Box>
    </Box>
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: theme.palette.background.default }}>
      ...
      {isMobile ? (
        <Drawer
          ...
          sx={{ '& .MuiDrawer-paper': { width: drawerWidth, border: 'none', bgcolor: '#1e293b', overflow: 'visible' } }}
        >
          {drawer}
        </Drawer>
      ) : (
