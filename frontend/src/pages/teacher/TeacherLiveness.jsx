...
export default function TeacherLiveness() {
  ...
  // Start/stop webcam when monitoring toggles
  useEffect(() => {
    if (monitoring) {
  ...
  const handleCapture = async () => {
    ...
    try {
      const r = await captureLiveness(selectedSection, photo)
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <Card sx={{ mb: 3 }}>
              <CardContent sx={{ p: 3 }}>
                ...
                {monitoring && (
                  <Box sx={{ mt: 2 }}>
                    {/* Hidden video feed for capturing frames */}
                    <video ref={videoRef} autoPlay playsInline muted
                      style={{ width: '100%', maxWidth: 480, borderRadius: 8, display: 'block', marginBottom: 12 }} />
