    ...
    <Box>
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        <Grid size={{ xs: 12, md: 8 }}>
          <Card sx={{ height: '100%' }}>
            <CardContent sx={{ p: 3 }}>
              ...
              <Typography variant="subtitle2" sx={{ mb: 1, color: 'text.secondary' }}>Send To:</Typography>
              <FormGroup row>
                <FormControlLabel
        ...
        <Grid size={{ xs: 12, md: 4 }}>
          <Card sx={{ height: '100%', background: theme.palette.mode === 'dark' ? 'linear-gradient(135deg, #0f2744, #0c1f3f)' : 'linear-gradient(135deg, #f0f9ff, #e0f2fe)' }}>
            <CardContent sx={{ p: 3 }}>
              <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, color: theme.palette.mode === 'dark' ? '#e0f2fe' : '#0c4a6e' }}>Report Contents</Typography>
              <List dense>
                {['Period-wise attendance', 'P/A matrix per student', 'Date range summary', 'Engagement metrics (if available)', 'Absent streak alerts'].map((item, i) => (
                  <ListItem key={i} disableGutters>
                    ...
                    <ListItemText primary={item} primaryTypographyProps={{ variant: 'body2', sx: { color: theme.palette.mode === 'dark' ? '#bae6fd' : '#0369a1' } }} />
                  </ListItem>
                ))}
              </List>
            </CardContent>
            ...
            <List>
              {schedules.map((sch) => (
                <motion.div key={sch.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  <ListItem
                    sx={{ borderRadius: 2, mb: 1, bgcolor: 'action.hover', border: '1px solid', borderColor: 'divider' }}
                  >
                    <ListItemText
                      primary={
