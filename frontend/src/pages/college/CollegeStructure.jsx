...
export default function CollegeStructure() {
  ...
  // Filters for branches tab
  const [branchBatchFilter, setBranchBatchFilter] = useState('')
  ...
  const filteredBranches = branches.filter(b => {
    if (branchBatchFilter && b.batch_id != branchBatchFilter) return false
    return true
  })
  ...
  // Branches scoped to a batch (used in class dialog/filter)
  const branchesForBatch = (batchId) => batchId
    ? branches.filter(b => b.batch_id == batchId)
    : branches
          ...
          <TableContainer component={Paper}>
            <Table size="small">
              ...
              <TableBody>
                ...
                {batches.length === 0 && <TableRow><TableCell colSpan={3} sx={{ textAlign: 'center', py: 3, color: 'text.disabled' }}>No batches yet.</TableCell></TableRow>}
              </TableBody>
            </Table>
          </TableContainer>
        </motion.div>
              ...
              <Grid container spacing={2} alignItems="center">
                <Grid size={{ xs: 12, sm: 4 }}>
                  <FormControl fullWidth size="small">
                    ...
                    <Select value={branchBatchFilter} label="Batch" onChange={e => setBranchBatchFilter(e.target.value)}>
                      <MenuItem value="">All Batches</MenuItem>
                      {batches.map(b => <MenuItem key={b.id} value={b.id}>{b.name} ({b.year})</MenuItem>)}
                    </Select>
                  </FormControl>
                </Grid>
                <Grid size={{ xs: 12, sm: 8 }} sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                  <Button variant="contained" startIcon={<Add />} onClick={() => { setDialog({ open: true, type: 'branch' }); setForm({ batch_id: branchBatchFilter }) }}>
                    Add Branch
                  </Button>
                </Grid>
              </Grid>
              ...
              <TableBody>
                {filteredBranches.map(b => (
                  <TableRow key={b.id}>
                    ...
                    <TableCell>{b.batch_name ? `${b.batch_name} (${b.batch_year})` : '-'}</TableCell>
                ...
                {filteredBranches.length === 0 && <TableRow><TableCell colSpan={6} sx={{ textAlign: 'center', py: 3, color: 'text.disabled' }}>No branches yet.</TableCell></TableRow>}
              </TableBody>
            </Table>
          </TableContainer>
        </motion.div>
            ...
            <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
              <Grid container spacing={2} alignItems="center">
                <Grid size={{ xs: 12, sm: 3 }}>
                  <FormControl fullWidth size="small">
                    <InputLabel>Batch</InputLabel>
                    <Select value={classBatchFilter} label="Batch" onChange={e => { setClassBatchFilter(e.target.value); setClassBranchFilter('') }}>
                      <MenuItem value="">All Batches</MenuItem>
                      {batches.map(b => <MenuItem key={b.id} value={b.id}>{b.name} ({b.year})</MenuItem>)}
                    </Select>
                  </FormControl>
                ...
                <Grid size={{ xs: 12, sm: 3 }}>
                  <FormControl fullWidth size="small" disabled={!classBatchFilter}>
                    <InputLabel>Branch</InputLabel>
                    <Select value={classBranchFilter} label="Branch" onChange={e => setClassBranchFilter(e.target.value)}>
                      <MenuItem value="">All Branches</MenuItem>
                      {branchesForBatch(classBatchFilter).map(b => <MenuItem key={b.id} value={b.id}>{b.name}</MenuItem>)}
                    </Select>
                  </FormControl>
                </Grid>
          ...
          <TableContainer component={Paper}>
            <Table size="small">
              ...
              <TableBody>
                ...
                {filteredClasses.length === 0 && <TableRow><TableCell colSpan={6} sx={{ textAlign: 'center', py: 3, color: 'text.disabled' }}>No classes found.</TableCell></TableRow>}
              </TableBody>
            </Table>
          </TableContainer>
        </motion.div>
            ...
            <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
              <Grid container spacing={2} alignItems="center">
                <Grid size={{ xs: 6, sm: 2.5 }}>
                  <FormControl fullWidth size="small">
                    <InputLabel>Batch</InputLabel>
                    <Select value={secBatchFilter} label="Batch" onChange={e => { setSecBatchFilter(e.target.value); setSecBranchFilter(''); setSecClassFilter('') }}>
                ...
                <Grid size={{ xs: 6, sm: 2.5 }}>
                  <FormControl fullWidth size="small" disabled={!secBatchFilter}>
                    <InputLabel>Branch</InputLabel>
                    <Select value={secBranchFilter} label="Branch" onChange={e => { setSecBranchFilter(e.target.value); setSecClassFilter('') }}>
                      <MenuItem value="">All</MenuItem>
                      {branchesForBatch(secBatchFilter).map(b => <MenuItem key={b.id} value={b.id}>{b.name}</MenuItem>)}
                    </Select>
                  </FormControl>
                </Grid>
                <Grid size={{ xs: 6, sm: 3 }}>
          ...
          <TableContainer component={Paper}>
            <Table size="small">
              ...
              <TableBody>
                ...
                {filteredSections.length === 0 && <TableRow><TableCell colSpan={4} sx={{ textAlign: 'center', py: 3, color: 'text.disabled' }}>No sections found.</TableCell></TableRow>}
              </TableBody>
            </Table>
          </TableContainer>
        </motion.div>
          ...
          <Grid container spacing={2} sx={{ mt: 0 }}>
            ...
            {dialog.type === 'class' && (<>
              <Grid size={{ xs: 6 }}>
                <FormControl fullWidth size="small">
                  <InputLabel>Batch</InputLabel>
                  <Select value={form.batch_id || ''} label="Batch" onChange={e => setForm({ ...form, batch_id: e.target.value, branch_id: '' })}>
                    <MenuItem value="">-- Select Batch --</MenuItem>
                    {batches.map(b => <MenuItem key={b.id} value={b.id}>{b.name} ({b.year})</MenuItem>)}
                  </Select>
                </FormControl>
              ...
              <Grid size={{ xs: 6 }}>
                <FormControl fullWidth size="small" disabled={!form.batch_id}>
                  <InputLabel>Branch</InputLabel>
                  <Select value={form.branch_id || ''} label="Branch" onChange={e => setForm({ ...form, branch_id: e.target.value })}>
                    <MenuItem value="">-- Select Branch --</MenuItem>
                    {branchesForBatch(form.batch_id).map(b => <MenuItem key={b.id} value={b.id}>{b.name}</MenuItem>)}
                  </Select>
                </FormControl>
              </Grid>
            </>)}
              ...
              <Grid size={{ xs: 12 }}>
                <FormControl fullWidth size="small">
                  ...
                  <Select value={form.class_id || ''} label="Class" onChange={e => setForm({ ...form, class_id: e.target.value })} required>
                    ...
                    {classes.map(c => <MenuItem key={c.id} value={c.id}>
                      {c.name}{c.branch_name ? ` · ${c.branch_name}` : ''}{c.batch_name ? ` · ${c.batch_name}` : ''}
                    </MenuItem>)}
                  </Select>
                </FormControl>
              </Grid>
