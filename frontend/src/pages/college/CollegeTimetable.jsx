...
import {
  ...
  DialogContent, DialogActions, FormControlLabel, Checkbox, Switch
} from '@mui/material'
...
export default function CollegeTimetable() {
  ...
  const [genForm, setGenForm] = useState({ start_time: '09:00', end_time: '16:00', num_periods: 6, enable_recess: true, recess_after: 3, recess_duration: 30, days: [0, 1, 2, 3, 4, 5] })
    ...
    <Box>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <TableContainer component={Paper} sx={{ overflow: 'auto' }}>
            <Table size="small" sx={{ minWidth: 800, borderCollapse: 'collapse' }}>
              <TableHead>
                <TableRow sx={{ bgcolor: 'text.primary' }}>
              ...
              <TableBody>
                {DAYS.map((dayName, dayIdx) => {
                  ...
                  return (
                    <TableRow key={dayIdx} sx={{ '&:hover': { bgcolor: 'action.hover' } }}>
                      <TableCell sx={{ fontWeight: 700, bgcolor: 'action.hover', border: '1px solid', borderColor: 'divider', textAlign: 'center' }}>
                        {DAY_SHORT[dayIdx]}
                      </TableCell>
                      {timeSlots.map((slot, slotIdx) => {
                        const period = dayPeriods.find(p => `${p.from_time}-${p.to_time}` === slot)
                        if (!period) {
                          return <TableCell key={slotIdx} sx={{ border: '1px solid', borderColor: 'divider', textAlign: 'center', color: 'text.disabled' }}>-</TableCell>
                        }
                        const isEditing = editingId === period.id
                        return (
                          <TableCell key={slotIdx} sx={{
                            border: '1px solid', borderColor: 'divider', textAlign: 'center', p: 0.5,
                            ...
                            {isEditing ? (
                              ...
                              <Box sx={{ cursor: 'pointer', py: 0.5 }} onClick={() => { setEditingId(period.id); setEditData(period) }}>
                                <Typography sx={{ fontWeight: 600, fontSize: '0.8rem', color: period.is_recess ? '#92400e' : 'text.primary' }}>
                                  {period.period_name || '-'}
                                </Typography>
                                {!period.is_recess && (
      ...
      <Dialog open={addOpen} onClose={() => setAddOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 0 }}>
            ...
            <Grid size={{ xs: 12 }}>
              <FormControlLabel
                control={<Checkbox checked={!!addForm.is_recess}
                  onChange={e => setAddForm({ ...addForm, is_recess: e.target.checked })} />}
                label="Mark as Recess / Break" />
            </Grid>
          </Grid>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={genOpen} onClose={() => setGenOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <Grid container spacing={2} sx={{ mt: 0 }}>
            <Grid size={{ xs: 6 }}>
              <TextField fullWidth size="small" label="School Start Time" type="time" slotProps={{ inputLabel: { shrink: true } }}
                value={genForm.start_time} onChange={e => setGenForm({ ...genForm, start_time: e.target.value })} />
            </Grid>
            <Grid size={{ xs: 6 }}>
              <TextField fullWidth size="small" label="School End Time" type="time" slotProps={{ inputLabel: { shrink: true } }}
                value={genForm.end_time} onChange={e => setGenForm({ ...genForm, end_time: e.target.value })} />
            </Grid>
            <Grid size={{ xs: 12 }}>
              <TextField fullWidth size="small" label="Number of Periods" type="number"
                value={genForm.num_periods} onChange={e => setGenForm({ ...genForm, num_periods: parseInt(e.target.value) })} />
            </Grid>
            <Grid size={{ xs: 12 }}>
              <FormControlLabel
                control={<Switch checked={genForm.enable_recess}
                  onChange={e => setGenForm({ ...genForm, enable_recess: e.target.checked })} />}
                label="Include Recess / Break" />
            </Grid>
            {genForm.enable_recess && (<>
              <Grid size={{ xs: 6 }}>
                <TextField fullWidth size="small" label="Recess After Period #" type="number"
                  value={genForm.recess_after} onChange={e => setGenForm({ ...genForm, recess_after: parseInt(e.target.value) })} />
              </Grid>
              <Grid size={{ xs: 6 }}>
                <TextField fullWidth size="small" label="Recess Duration (min)" type="number"
